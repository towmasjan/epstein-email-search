...
def test_is_translation_valid():
    ...
    assert is_translation_valid("Hello", "Witaj") is True, "Poprawne tłumaczenie powinno być valid"

    # Test 2: Tłumaczenie takie samo jak oryginał
    assert is_translation_valid("Hello", "Hello") is False, "Tłumaczenie identyczne z oryginałem powinno być invalid"

    # Test 3: Puste tłumaczenie
    assert is_translation_valid("Hello", "") is False, "Puste tłumaczenie powinno być invalid"

    # Test 4: None jako tłumaczenie
    assert is_translation_valid("Hello", None) is False, "None jako tłumaczenie powinno być invalid"
...
def test_is_pipeline():
    ...
    # Test 1: None
    assert is_pipeline(None) is False, "None nie powinno być pipeline"

    # Test 2: Zwykła funkcja
    def test_func():
        pass

    assert is_pipeline(test_func) is False, "Zwykła funkcja nie powinna być pipeline"
    ...
    assert is_pipeline(mock_pipeline) is True, "Obiekt z model i tokenizer powinien być pipeline"
